...
| `Session X: content hash mismatch` | The raw chat file's content no longer matches the hash recorded when it was first collected | The raw file was edited or corrupted after collection — re-collect from the original source if possible |
