...
|---|---|---|
| ~~No TLS on `server.py`~~ | Session cookie crossed the LAN in cleartext | **RESOLVED 2026-08-05 (desktop)** | Self-signed cert added (`~/.claude-search-library/certs/`), HTTPS is now the default, `--no-tls` for plain-HTTP dev. See CLAUDE.md Known Blockers. |
...
| ~~114 stale pre-migration files in `claude-search-data`~~ | Dead weight in `encrypted_sessions/`/`encrypted_summaries/` | **RESOLVED 2026-08-05 (desktop)** | Deleted after confirming nothing in `sync.py` reads them — fully superseded by the `changesets/` stream. |
